...
async def gen_mind_map(client, nb_id):
    print("    🧠 mind map...")
    # generate_mind_map retorna dict directo (no GenerationStatus)
    result = await client.artifacts.generate_mind_map(nb_id, language="es")
    return result
...
async def process(client, unit, skip_audio=False):
    ...
    # Mind map: API genera+espera internamente, descargamos luego a file JSON
    try:
        print("    🧠 mind map...")
        await client.artifacts.generate_mind_map(nb.id, language="es")
        with tempfile.NamedTemporaryFile(suffix=".json", delete=False) as tmp:
            tmp_path = Path(tmp.name)
        await client.artifacts.download_mind_map(nb.id, str(tmp_path))
        import json
        updates["mind_map_json"] = json.loads(tmp_path.read_text())
        tmp_path.unlink()
        print("      ✅ mindmap → DB JSON")
    except Exception as e:
        print(f"      ⚠ mindmap falló: {type(e).__name__}: {str(e)[:120]}")
    ...
    try:
        st = await client.artifacts.generate_study_guide(nb.id, language="es")
